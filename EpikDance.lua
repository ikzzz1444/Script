...
game:GetService("StarterGui"):SetCore("SendNotification", { 
	Title = "supah epik dancez";
	Text = "credits to gObl00x";
...
-- Create the tool
local tool = Instance.new("Tool")
tool.Name = "door stuck"
tool.RequiresHandle = false
tool.Parent = backpack

        writefile("door stuck.mp3", game:HttpGet("https://github.com/imjustmessingaround418/music-modded/raw/refs/heads/main/door%20stuck%20(mp3cut.net).mp3"))
-- Load Animator if not already loaded
if not getgenv()["Animator"] then
    loadstring(game:HttpGet("https://raw.githubusercontent.com/xhayper/Animator/main/Source/Main.lua"))()
    end
...
tool.Equipped:Connect(function()
    local character = player.Character
    if character then
        Anim = Animator.new(character, 83291488848363)
        Anim:Play()
        Anim.Stopped:Connect(function()
            Anim:Play()
        end)
        sound = Instance.new("Sound")
        sound.SoundId = getcustomasset("door stuck.mp3")
        sound.Volume = 2
        sound.Looped = true
        sound.Parent = game.Players.LocalPlayer.Character:WaitForChild("HumanoidRootPart")
        sound:Play()
...
tool.Equipped:Connect(function()
    local character = player.Character
    if character then
        Anim = Animator.new(character, 112844131485001)
        Anim:Play()
        Anim.Stopped:Connect(function()
            Anim:Play()
        end)
