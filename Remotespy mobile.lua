...
local nextb = Create("TextButton", {Parent = gui})
...
nextb.Size = UDim2.new(0,35,0,35)
...
function connectResize()
    ...
    local lastCam = SafeGetService("Workspace").CurrentCamera:GetPropertyChangedSignal("ViewportSize"):Connect(bringBackOnResize)
    SafeGetService("Workspace"):GetPropertyChangedSignal("CurrentCamera"):Connect(function()
        lastCam:Disconnect()
        if typeof(lastCam) == 'Connection' then
            lastCam:Disconnect()
        end
        lastCam = SafeGetService("Workspace").CurrentCamera:GetPropertyChangedSignal("ViewportSize"):Connect(bringBackOnResize)
    end)
end
